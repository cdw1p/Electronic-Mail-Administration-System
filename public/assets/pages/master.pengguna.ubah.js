...
new Vue({
  ...
  methods: {
    ...
    postUpdateUsers: async function () {
      Swal.fire({ title: 'Apakah Anda yakin ingin melanjutkan ?', showCancelButton: true, confirmButtonText: 'Lanjutkan', cancelButtonText: 'Batalkan' }).then(async (result) => {
        if (result.isConfirmed) {
          try {
            ...
            const makeRequest = await axios.post('/api/master/updateUsers', this.inputForm, { headers: { 'authorization': `Bearer ${this.userSession.access_token}` } })
